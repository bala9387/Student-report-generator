...
(function () {
  ...
  function renderReport(rep, model) {
    ...
    // 7. Core Subject Concepts Tested
    if (rep.coreConcepts && rep.coreConcepts.length) {
      h += "<div class='pdf-sec-bar'>6. Core Subject Concepts Tested</div>";
      h += renderBulletList(rep.coreConcepts);
    }

    // 8. Subject-Specific Study Tips & Strategies
    if (rep.studyTips && rep.studyTips.length) {
      h += "<div class='pdf-sec-bar'>7. Subject-Specific Study Tips &amp; Strategies</div>";
      h += renderBulletList(rep.studyTips);
    }
