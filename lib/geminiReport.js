...
async function generateWithGroq(inputs, groqKey) {
  ...
  function extractText(label, input) {
    if (!input) return "";
    let str = "\n===== " + label + " =====\n";
    if (input.text) str += input.text;
    else if (input.name) str += "[Document file uploaded: " + input.name + "]";
    return str;
  }

  promptText += extractText("SYLLABUS", inputs.syllabus);
  promptText += extractText("QUESTION PAPER", inputs.questionPaper);
  promptText += extractText("STUDENT WRITTEN ANSWER SHEET", inputs.answerPaper);
