...
async function generateReport(inputs) {
  ...
  if (apiKeys.length === 0 && !groqKey) {
    const e = new Error('No AI servers are currently configured.');
    e.status = 500;
    throw e;
  }
    ...
    for (let step = 0; step < totalKeys; step++) {
      ...
      for (const model of candidateModels) {
        try {
          ...
          lastError = err;
          // No delay needed when rotating to the next key, since each key has its own independent Google rate limit.
        }
      }
    }
  }
  ...
  if (lastError && (lastError.status === 429 || (lastError.message && lastError.message.toLowerCase().includes('quota')))) {
    const cleanErr = new Error('The AI evaluation servers are currently busy due to high traffic. Please wait a few moments and try again.');
    cleanErr.status = 429;
    throw cleanErr;
  }
