...
async function checkApiHealth() {
  ...
  if (!geminiKey || !geminiKey.trim()) {
    return { status: "missing_key", message: "No API Key configured" };
  }
  try {
    const testModel = process.env.GEMINI_MODEL || 'gemini-flash-latest';
    const testRes = await fetch(GEMINI_ENDPOINT(testModel, geminiKey.trim()), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    if (!testRes.ok) {
      ...
      if (msg.includes("prepayment") || msg.includes("depleted")) {
        return { status: "depleted", code: 429, message: "Prepayment credits depleted" };
      }
      return { status: "rate_limited", code: testRes.status, message: msg };
    }
    return { status: "active", message: "Active & Ready" };
  } catch (err) {
  ...
  for (let kIdx = 0; kIdx < keys.length; kIdx++) {
    ...
    for (const model of candidateModels) {
      try {
        ...
        if (err.message && err.message.includes('prepayment')) {
          break;
        }
      }
    }
