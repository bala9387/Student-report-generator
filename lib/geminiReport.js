...
async function generateWithKimi(inputs, key) {
  const endpoint = process.env.KIMI_ENDPOINT || 'https://api.moonshot.cn/v1/chat/completions';
  ...
  if (inputs.syllabus) {
    if (inputs.syllabus.text) userText += "\n===== SYLLABUS =====\n" + inputs.syllabus.text;
    else if (inputs.syllabus.data && inputs.syllabus.mimeType) {
      contentParts.push({ type: "image_url", image_url: { url: "data:" + inputs.syllabus.mimeType + ";base64," + inputs.syllabus.data } });
    }
  }

  if (inputs.questionPaper) {
    if (inputs.questionPaper.text) userText += "\n===== QUESTION PAPER =====\n" + inputs.questionPaper.text;
    else if (inputs.questionPaper.data && inputs.questionPaper.mimeType) {
      contentParts.push({ type: "image_url", image_url: { url: "data:" + inputs.questionPaper.mimeType + ";base64," + inputs.questionPaper.data } });
    }
  }

  if (inputs.answerPaper) {
    if (inputs.answerPaper.text) userText += "\n===== STUDENT WRITTEN ANSWER SHEET =====\n" + inputs.answerPaper.text;
    else if (inputs.answerPaper.data && inputs.answerPaper.mimeType) {
      contentParts.push({ type: "image_url", image_url: { url: "data:" + inputs.answerPaper.mimeType + ";base64," + inputs.answerPaper.data } });
    }
  }
  ...
  userText += "\n\nOutput strictly a valid JSON object matching the requested evaluation schema. No surrounding markdown.";
  ...
  const raw = await res.text();
  if (!res.ok) {
    let msg = "Kimi API error (HTTP " + res.status + ")";
    try { const j = JSON.parse(raw); if (j.error && j.error.message) msg = j.error.message; } catch (e) {}
    const err = new Error(msg);
    err.status = res.status;
    throw err;
  ...
  if (!textOut) throw new Error("Kimi returned empty content.");

  const cleanJson = textOut.replace(/^```json\s*/i, '').replace(/```\s*$/i, '').trim();
  return { report: JSON.parse(cleanJson), model: "Kimi (" + model + ")" };
}

// ---------- Multi-Engine Load Balancer & Failover Router ----------
let requestCount = 0;
...
async function generateReport(inputs) {
  ...
  if (!geminiKey && !kimiKey) {
    const e = new Error('No API keys configured on the server (GEMINI_API_KEY or KIMI_API_KEY).');
    e.status = 500;
    throw e;
  }
  ...
  const hasBinaryPdf = (inputs.syllabus && inputs.syllabus.data && inputs.syllabus.mimeType === 'application/pdf') ||
                       (inputs.questionPaper && inputs.questionPaper.data && inputs.questionPaper.mimeType === 'application/pdf') ||
                       (inputs.answerPaper && inputs.answerPaper.data && inputs.answerPaper.mimeType === 'application/pdf');

  requestCount++;
  // Alternate primary provider between Gemini and Kimi (unless binary PDF is present)
  const preferKimi = hasBinaryPdf ? false : ((kimiKey && geminiKey) ? (requestCount % 2 === 0) : !!kimiKey);

  const providers = [];
  if (preferKimi) {
    if (kimiKey) providers.push({ name: "Kimi", fn: () => generateWithKimi(inputs, kimiKey) });
    if (geminiKey) providers.push({ name: "Gemini", fn: () => generateWithGemini(inputs, geminiKey) });
  } else {
    if (geminiKey) providers.push({ name: "Gemini", fn: () => generateWithGemini(inputs, geminiKey) });
    if (kimiKey && !hasBinaryPdf) providers.push({ name: "Kimi", fn: () => generateWithKimi(inputs, kimiKey) });
  }

  let lastErr;
  for (const p of providers) {
    try {
      console.log(`[Report Engine] Evaluating request #${requestCount} with ${p.name}...`);
      return await p.fn();
    } catch (err) {
      console.warn(`[Report Engine] ${p.name} failed (${err.message}). Trying backup provider...`);
      lastErr = err;
    }
  }

  throw lastErr || new Error("Evaluation failed on all AI providers.");
}
