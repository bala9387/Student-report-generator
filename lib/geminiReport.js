  ...
  for (let kIdx = 0; kIdx < keys.length; kIdx++) {
    ...
    for (const testModel of testModels) {
      try {
        ...
        if (testRes.status === 402 || msg.includes("prepayment") || msg.includes("depleted")) {
          lastStatus = "depleted";
          lastCode = 402;
  ...
  for (let kIdx = 0; kIdx < keys.length; kIdx++) {
    ...
    for (const model of candidateModels) {
      try {
        ...
        lastErr = err;
        // If credits depleted on this key, immediately skip to next API key
...
module.exports = { generateReport, checkApiHealth };
