...
// ---------- Gemini Provider ----------
async function generateWithGemini(inputs, key, modelName, retries = 2) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (!res.ok) {
        ...
        try {
          ...
          if (j.error && j.error.message) {
            ...
            if (match && match[1]) {
              retrySeconds = Math.ceil(parseFloat(match[1])) + 1;
            }
          }
        } catch (e) {}

        const isRateLimit = res.status === 429 || msg.toLowerCase().includes('quota') || msg.toLowerCase().includes('rate');
        if (isRateLimit && attempt < retries) {
          console.warn(`[Report Engine] Rate limit hit on ${model}. Waiting ${retrySeconds}s before retry ${attempt + 1}/${retries}...`);
      ...
      if (err.status === 429 || (err.message && (err.message.includes('Quota') || err.message.includes('rate')))) {
        await sleep(5000);
      } else {
        throw err;
      }
