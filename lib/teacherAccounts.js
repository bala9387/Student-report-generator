/* Staff / Teacher Accounts Registry with Role-Based Subject Access Control */
const ACCOUNTS = {
...
function verifyTeacherLogin(userInput, passInput) {
  ...
  if (user === expectedUser && pass === expectedPass) {
    return { ok: true, name: 'Master Administrator', user: expectedUser, allowedCodes: null, isAdmin: true };
  }

  const acc = ACCOUNTS[user];
  if (acc && acc.password === pass) {
    return { ok: true, name: acc.name, user: user, subjects: acc.subjects, allowedCodes: acc.allowedCodes || null, isAdmin: false };
  }
  return false;
}
