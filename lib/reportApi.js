...
async function getLookup(mode, rollRaw, grade) {
  ...
  try {
    ...
    if (mentorNode && mentorNode.links) {
      if (EXAMS.indexOf(mode) !== -1) {
        mentorReportUrl = mentorNode.links[mode] || null;
      } else {
        // For PE - Analysis, pick the most recent available link
        mentorReportUrl = mentorNode.links["TE 2"] || mentorNode.links["CU 2"] || mentorNode.links["TE 1"] || mentorNode.links["CU 1"] || null;
      }
    }
