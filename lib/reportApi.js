...
const CACHE_TTL = 1 * 60 * 1000; // re-fetch the sheet at most once every 1 minute
...
async function getData(grade) {
  ...
  if (cachedDataByGrade[g] && (Date.now() - (lastFetchByGrade[g] || 0) < CACHE_TTL)) {
...
async function getLookup(mode, rollRaw, grade) {
...
async function getLeaderboard(scope, nRaw, grade) {
