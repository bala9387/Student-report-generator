    ...
    if (st) {
      ...
      mode.subjects.forEach((s, i) => {
        if (upd.marks && upd.marks[s] !== undefined && isSubjectAllowed(s)) {
          ...
          const val = (isEmpty || isAB) ? null : parseFloat(raw);
          st.marks[examName][s] = isAB ? "AB" : val;
          if (val != null && !isNaN(val)) sum += val;
          
          if (st.rowIdx && baseCol != null) {
            const colLetter = colIndexToLetter(baseCol + i);
            const sheetVal = isAB ? "AB" : (val == null ? "" : val);
            sheetUpdates.push({
              range: `'${streamTabName}'!${colLetter}${st.rowIdx}`,
              values: [[sheetVal]]
            });
          }
      ...
      if (st.rowIdx && baseCol != null) {
        ...
        sheetUpdates.push({
          range: `'${streamTabName}'!${totalColLetter}${st.rowIdx}`,
          values: [[sum]]
        });
      }
      ...
      if (peMode && isG12) {
        ...
        if (peSt) {
          ...
          if (peSt.rowIdx && PE_COLS[examName] != null) {
            const peColLetter = colIndexToLetter(PE_COLS[examName]);
            sheetUpdates.push({
              range: `'PE - Analysis'!${peColLetter}${peSt.rowIdx}`,
              values: [[sum]]
            });
          }
        }
    ...
    if (peSt && upd.exams) {
      EXAMS.forEach(ex => {
        if (upd.exams[ex] !== undefined) {
          ...
          if (peSt.rowIdx && PE_COLS[ex] != null && String(grade || "12").trim() === "12") {
