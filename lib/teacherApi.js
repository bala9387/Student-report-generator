...
async function updateStudentMarks(streamName, examName, updates, allowedCodes, allowedStreams, grade) {
  ...
  const sheetUpdates = [];
  const BLOCK_START_G12 = { "CU 1": 3, "TE 1": 11, "TE 2": 27 };
  const BLOCK_START_DEFAULT = { "CU 1": 3, "TE 1": 10, "TE 2": 24 };
  const BLOCK_START = isG12 ? BLOCK_START_G12 : BLOCK_START_DEFAULT;
          ...
          if (baseCol != null) {
            ...
            if (isG12) {
              if (i < 5) offset = i;
                ...
                if (streamTabName === "Bio - Maths" && (examName === "CU 1" || examName === "TE 2")) {
                  offset = 5;
                } else {
                  offset = 6;
                }
