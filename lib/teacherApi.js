...
async function getSheetsClient() {
  const email = (process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL || DEFAULT_EMAIL).trim().replace(/^["']|["']$/g, '');
  const rawKey = process.env.GOOGLE_PRIVATE_KEY || DEFAULT_KEY;
  const key = fixPrivateKey(rawKey);
  ...
  try {
    ...
    throw new Error(`Google Auth Failed (${err.message}). Please verify that GOOGLE_SERVICE_ACCOUNT_EMAIL and GOOGLE_PRIVATE_KEY are set correctly in Netlify.`);
  }
}
