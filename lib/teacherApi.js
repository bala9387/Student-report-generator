...
async function getSheetsClient() {
  ...
  let key = (process.env.GOOGLE_PRIVATE_KEY || '').trim().replace(/^["']|["']$/g, '');
  ...
  try {
    ...
    throw new Error(`Google Authentication Failed: ${err.message}. Please check GOOGLE_PRIVATE_KEY format in Netlify.`);
  }
}
